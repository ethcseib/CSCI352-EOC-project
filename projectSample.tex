...
\begin{abstract}
This project is a 2D game named ``Burning Choices". The game will primarily focus on the adventure genre. It will include multiple levels and a branching story. The game took inspiration from Bandersnatch, a fictitious game in the Netflix original series Black Mirror that also included a branching story line. The target audience will be those that enjoy classic action arcade style games.
\end{abstract}
...
\hline
Task ID & Task Name & Primary Actor & Complexity & Priority \\
\hline 
...
\begin{itemize}
\item[Task Number:] 1
\item[Task Name:] Starting the story.
...
\begin{enumerate}
\item[Task Number:] 5
\item[Task Name:] Character Movement
...
\begin{enumerate}
\item[Task Number:] 6
\item[Task Name:] Items
\item[Description:] To complete quests the player needs to collect items.
\end{enumerate}

\begin{enumerate}
\item Player has to collect items for a quest.
\item The player walks onto an item.
\item The item is removed from the level.
\item The collected item is moved to the player's inventory.
\end{enumerate}

\begin{enumerate}
\item[Task Number:] 7
\item[Task Name:] Walls
\item[Description:] An object the player can not walk through.
\end{enumerate}

\begin{enumerate}
\item The player tries to walk through a wall.
\item The walls will stop their advance in that direction.
\end{enumerate}
...
Next is the verification phase and it will last from April 21, 2019 through April 22, 2019. It is at this point we will begin trying to break the game in order to find bugs in the project.

Lastly is the Maintenance phase which will last from April 22, 2019 to April 24, 2019. We will take this time to fix any bugs we have discovered.

It's hard to tell whether we budgeted our time well. We have the majority of the gameplay implemented and waiting to be used, but there are still many things like the main menu, pause menu and the story/ levels that need to be finished. The main menu and pause menu have their own functionality that is separate from the gameplay that has yet to be defined. With these tasks taken in consideration with the fact that the deadline is close. It is safe to say that the time was not budgeted well enough. The deadline is too close for comfort. This has us nervous which means there is more we could have done but didn't.
...
The second pattern we are using is the Observer pattern. This keeps the inventory system up to date when an item is collected. 
...
\section{Results}
The project's gameplay functionality is nearly complete. This means the player can collect items to the inventory and the player can't travel to places they aren't allowed to. The game can load in a new level when the player progresses through the story. We have the capability to create the story within the game as well as little quests/ missions to complete in the game, but we have not taken the time to implement them into the game.
